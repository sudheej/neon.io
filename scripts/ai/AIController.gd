...
func _process(delta: float) -> void:
	...
	if player.has_method("set_ai_move_command"):
		player.set_ai_move_command(move)
...
func _maybe_pick_weapon(owner: Node2D, targets: Array, _delta: float) -> void:
	...
	if system.get_weapon_ammo(weapon_type) <= 0:
		system.select_weapon_and_buy(weapon_type)
	else:
		system.try_set_selected_weapon(weapon_type)
